...
test("X and LinkedIn use the same background authorization, stored settings, and classification broker", async () => {
  const state = {
    typesafeApiKey: "test-key",
    hiddenTweetCategories: ["grift"],
    anonymizeSocialPosters: true
  };
  ...
  for (const url of ["https://x.com/home", "https://twitter.com/home", "https://www.linkedin.com/feed/"]) {
    ...
    assert.equal((await background.message({ type: "get-anonymize-social-posters" }, url)).enabled, true);
...
test("filters, privacy, and retries broadcast to both platforms", async () => {
  ...
  assert.equal(background.sent.filter(({ message }) => message.type === "apply-anonymize-social-posters").length, 2);
